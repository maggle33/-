...
(function (global) {
  ...
  function renderTimetable() {
    ...
    // 课表只允许纵向滚动：横向尺寸由 7 个星期列在容器内自适应，
    // 同时把滚动方向标记给 iOS 适配层，避免双指缩放/横向拖拽破坏课表比例。
    html += '<div class="card tt-card"><div class="tt-scroll" data-scroll-axis="vertical"'
      + ' style="touch-action:pan-y;overflow-x:hidden;overflow-y:auto"><div class="tt-grid">'
  ...
  /** 打开编辑面板时以当前可见视口为基准，避免 iOS 键盘/页面缩放后面板落在底部。 */
  function showEditPanel() {
    renderPanel();
    const panel = $('panel');
    panel.classList.add('on');
    panel.scrollTop = 0;
    // 等浏览器完成 fixed 面板布局后再归零一次，兼容 iOS Safari 的视口重排。
    global.requestAnimationFrame(function () {
      panel.scrollTop = 0;
      if (global.CR && global.CR.ios && typeof global.CR.ios.syncPanel === 'function') {
        global.CR.ios.syncPanel();
      }
    });
  }
